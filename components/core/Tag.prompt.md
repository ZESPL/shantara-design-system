...
**Motion.** Hover fills Pearl Bush (or Himalaya-800 when selected) at 160ms on fine pointers; press is `scale(var(--press-scale))` (`0.96`). With `onClick` the tag is a button — Enter and Space toggle it. The remove control fades to full opacity on hover. Static tags (`data-static`) do not press.
