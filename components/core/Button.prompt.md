...
```jsx
<Button variant="primary" size="lg" endIcon={<Icon name="arrow-right" />}>Book a stay</Button>
```
...
**Motion.** Hover is a one-step fill or hairline shift at `--duration-fast` (160ms), gated to fine pointers. Press is `scale(0.97)` on `:active`, never a colour flash. Focus is the 2+2px Himalaya ring. Disabled sits at 42% and does not move.
