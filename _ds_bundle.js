...
function initialsOf(name) {
  return String(name || "").replace(/^(Dr\.?|Mr\.?|Ms\.?|Mrs\.?)\s+/i, "").split(/[\s.]+/).filter(Boolean).map(p => p[0]).slice(-2).join("").toUpperCase();
}
