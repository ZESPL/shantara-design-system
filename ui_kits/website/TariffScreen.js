        ...
        <Photo name="room-premium" alt={t("Rooms and tariffs")} height={420} radius="0px" scrim="bottom">
          <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "flex-end" }}>
            <div style={{ maxWidth: "var(--layout-max)", width: "100%", margin: "0 auto", padding: "0 var(--layout-gutter-lg) var(--space-11)" }}>
              ...
              <h1 style={{ font: "var(--type-h1)", color: "var(--color-merino)", margin: "var(--space-4) 0 var(--space-5)", maxWidth: "14ch" }}>{t("Restraint, not excess")}</h1>
              <p style={{ font: "var(--type-lead)", color: "color-mix(in srgb, var(--color-merino) 88%, transparent)", maxWidth: "42ch", margin: 0 }}>{t("Fifty-two rooms across five categories, six floors, and mountain views from some rooms in every one of them.")}</p>
            </div>
          </div>
        </Photo>
      </section>
      <div style={{ maxWidth: "var(--layout-max)", margin: "0 auto", padding: "0 var(--layout-gutter-lg) var(--section-y)" }}>
      <Breadcrumbs items={[{ label: t("Stay"), href: "#" }, t("Rooms & tariffs")]} />
      <div style={{ display: "flex", gap: "var(--space-11)", marginTop: "var(--space-7)", alignItems: "flex-start", flexWrap: "wrap" }}>
        <div style={{ flex: "1 1 520px", minWidth: 0 }}>
          <span className="shantara-eyebrow">{t("Tariff card")}</span>
          <h2 style={{ font: "var(--type-h1)", fontSize: "var(--text-3xl)", margin: "var(--space-4) 0 var(--space-6)" }}>{t("Rooms and tariffs")}</h2>
          <p style={{ font: "var(--type-lead)", margin: "0 0 var(--space-8)", maxWidth: "52ch" }}>{t("Five categories. One inclusive figure per night. Nothing is added at checkout.")}</p>
          {TARIFF.map((r) => (
          ...
          <p style={{ fontSize: "var(--text-sm)", color: "var(--text-secondary)", margin: "var(--space-7) 0 0", maxWidth: "72ch" }}>{t("Rates valid to 31 December 2026. Reservations are confirmed only after a mandatory preliminary consultation.")}</p>
        </div>
        <Card padding="lg" style={{ flex: "0 1 360px" }}>
          <span className="shantara-eyebrow">{t("What every tariff includes")}</span>
          <h2 style={{ font: "var(--type-h4)", margin: "var(--space-4) 0 var(--space-6)" }}>{t("One price, nothing added later")}</h2>
          ...
          <Button fullWidth size="lg" onClick={() => onNavigate("booking")}>{t("Request a consultation")}</Button>
